...
import { FileText, Clock, Star, ChevronDown } from 'lucide-react';

const icons = [FileText, Clock, Star];
const colors = ['bg-primary/10 text-primary', 'bg-accent/10 text-accent', 'bg-chart-3/20 text-yellow-600 dark:text-yellow-400'];

export default function BecomeSteps() {
    const { t } = useLanguage();
    const [activeStep, setActiveStep] = useState(null);

    const steps = [
        { title: t('becomeSteps.step1Title'), desc: t('becomeSteps.step1Desc') },
        { title: t('becomeSteps.step2Title'), desc: t('becomeSteps.step2Desc') },
        { title: t('becomeSteps.step3Title'), desc: t('becomeSteps.step3Desc') },
    ];

    return (
        <div className="mb-8">
            <h2 className="text-xl font-bold text-foreground mb-1">{t('becomeSteps.title')}</h2>
            <p className="text-sm text-muted-foreground mb-4">{t('becomeSteps.subtitle')}</p>
